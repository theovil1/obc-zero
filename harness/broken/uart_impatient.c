 ...
 *
 * harness/broken/uart_stalled.c that gives up on the first refusal instead of
 * spending its allowance.
 *
 * This is the build the cost floor exists for. Every *behavioural* assertion
 * passes here — the frame is shed, the drop is counted and announced, the task
 * stays inside its budget, the machine boots once — because giving up instantly
 * does all of those things, and does them cheaply. What it does not do is wait,
 * so a port that would have cleared in twenty polls costs a frame that did not
 * need to be lost.
 *
 * A mechanism that is correct in everything the observation covers and wrong in
 * the thing nobody thought to observe. The floor is the observation.
 *
...
static obc_status_t port_putc(uint32_t base, uint8_t byte, uint32_t *allowance)
{
    ...
    for (;;) {
        ...
        /* BROKEN: the allowance is never spent. */
        (void)allowance;
