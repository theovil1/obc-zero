 ...
 *
 * A copy of flight/tlm/sensor.c with both detectors removed: every reading is
 * published as valid, whatever it is. This is the validator that would make the
 * "flagged" half of the criterion vacuous.
 *
 * It exists because a test that has never failed has not been shown to work. The
 * range and stuck cases in test-tlm-sensors pass against the real validator, and
 * that says nothing until they are shown to fail against one that does nothing.
 *
...
obc_status_t obc_sensor_sample(void)
{
    ...
    for (i = 0u; i < OBC_SENSOR_COUNT; i++) {
        ...
        /* BROKEN: both detectors deleted. Nothing is ever rejected. */
