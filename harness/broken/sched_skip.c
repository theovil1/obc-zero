 ...
 *
 * The defect is chosen to be as unremarkable as possible: the system boots,
 * every task runs, the frames are waited out, the slack is healthy, and the
 * serial log looks correct. Only the count and the order betray it. A scheduler
 * wrong in exactly this way would run for months.
 *
 ...
 * flight/. It is regenerated from the flight executive whenever that changes
 * shape — it went stale once already when M3 added safe mode, and the build
 * failed loudly, which is the intended behaviour of a copy.
 *
