 ...
 *
 * A copy of flight/tlm/sensor.c that rejects every reading, including good ones.
 *
 * This is the build the *second* direction exists for, and it is the one a
 * careless checker passes. "A bad reading is flagged" holds perfectly here — the
 * validator flags everything — and a system running this would spend an entire
 * mission reporting two dead sensors that were both working. Only the "unflagged
 * means real" direction can tell the two apart.
 *
...
obc_status_t obc_sensor_sample(void)
{
    ...
    for (i = 0u; i < OBC_SENSOR_COUNT; i++) {
        ...
        /* BROKEN: the bound is ignored and every reading is called out of
